...
def excute_adb(cmd, serial=None):
    ...
    #print "{0}: {1}".format(command, file.readlines())
    return file
...
def excute_adb_process(cmd, serial=None):
    ...
    print "{0}: {1} err: {2}".format(command, out, erro)
    return out
...
# def _call_adb_shell(*args):
#     cmd_line = ["adb"]
#     cmd_line.extend(*args)
#     logger.debug("start process {0}".format(cmd_line))
#     returnvalue = subprocess.call(cmd_line)
#     logger.debug("subprocess {0},exit code {1}".format(cmd_line, returnvalue))

def _call_adb_shell(cmdline):
    while True:
        logger.debug("start cmd :{0}".format(cmdline))
        os.system(cmdline)
        time.sleep(2)

def call_adb_shell(*args):
    thread.start_new_thread(_call_adb_shell, args)
